...
//contexto 2d
const canvas2 = document.querySelector("#canvas2d")  as HTMLCanvasElement | null;
if (canvas2 === null) throw new Error("Could not find canvas element");
const contexto2D = canvas2.getContext("2d");
if (contexto2D === null) throw new Error("Could not get WebGL context");
...
class Game{
  ...
  async init(gl : WebGL2RenderingContext){
    this.gl = gl;
    await WebGLController.init(this.gl);
  ...
  async initScene(gl : WebGL2RenderingContext){
    this.numeroPersonajeAndando = 0;
    ...
    for(let i = 0; i < posHabitaciones.length; i += 2){
      ...
      await this.lamparas[this.lamparas.length-1].cargarModelo(gl);
    ...
    let pos = this.map.getCasillaVacia();//y,x
    ...
    await this.protagonist.cargarModelo(gl);

    await this.arco.cargarModelo(gl);
    ...
    for(let i = 1; i < randNumEnemigos; i++){
      ...
      if(randEnemigo == 1) enemy = new Enemy(i, "Enemigo1",["Parado","Andar","Atacar","Daño"],[30,30,30,30]);
      if(randEnemigo == 2) enemy = new Enemy(i, "Enemigo2",["Parado","Andar","Atacar","Daño"],[30,30,30,30]);

      await enemy.cargarModelo(gl);
      pos = this.map.getCasillaVacia();
      enemy.setCasilla(pos[0],pos[1]);
      enemy.setPosition(pos[1]*2,0,pos[0]*2);
      if(randEnemigo == 1)enemy.setRotateY(90);
    ...
    for(let y = 0; y < this.map.tamMapa; y++){
      for(let x = 0; x < this.map.tamMapa; x++){
        if(this.map.comprobarCasillaEsPared(y,x)){
          ...
          await this.paredes[this.paredes.length-1].cargarModelo(gl);
          this.paredes[this.paredes.length-1].setPosition(x*2,0,y*2);
          this.paredes[this.paredes.length-1].setRotateY(Matematicas.RandInteger(0,3)*90);//Para que muestren una cara aleatoria.
        }
    ...
    this.escaleras = new Character(4, "Escaleras",["Parado"],[1]);
    await this.escaleras.cargarModelo(gl);
    let posEscalera = this.map.getPosParaEscaleras(); //y,x,orientacion
  ...
  async logic(){
      switch(this.stateGamePlay){
        ...
        break;
        case 1: 
          await this.initScene(this.gl)
          this.stateGamePlay = 2;
        break;
        case 2:
          if(this.turnos.turnoActual == 0)this.actionsProtagonist();
          else this.actionsEnemy();
  ...
  renderGame(){
    ...
    if(this.stateGamePlay==2) setTimeout(this.renderGame.bind(this) ,1);
    if(this.stateGamePlay==3) this.renderScreenPiso();
    if(this.stateGamePlay==4) this.renderScreenWin();
    if(this.stateGamePlay==5) this.renderScreenLose();
  }
  ...
  renderInterface(){
    ...
    if(this.stateGamePlay==2) setTimeout(this.renderInterface.bind(this) ,100);//refresco si estoy en estado de gameplay
  }

  renderScreenInit(){
    this.screens.drawPantallaInicio();

    if(this.stateGamePlay == 0) setTimeout(this.renderScreenInit.bind(this) ,100); //si estoy en mi estado me actualizo
    else this.renderScreenLoading();

  }
  renderScreenLoading(){
    this.screens.drawPantalla("CARGANDO");

    if(this.stateGamePlay == 1) setTimeout(this.renderScreenLoading.bind(this) ,100);
  ...
  renderScreenWin(){
    ...
    if(this.stateGamePlay == 0)this.renderScreenInit();
    else setTimeout(this.renderScreenWin.bind(this) ,100);
  }  
  renderScreenLose(){
    this.screens.drawPantalla("YOU LOSE");
    if(this.stateGamePlay == 0)this.renderScreenInit();
    else setTimeout(this.renderScreenLose.bind(this) ,100);
  }  
    ...
    switch(this.protagonist.getState()){
      case 0:
        ...
        if(actionSelect == 0){
          ...
          if(this.protagonist.usarPocion() == 1)this.turnos.actualizarTurnos(); 
        }
      ...
      case 2:
        if(this.atacar() == 0) this.turnos.actualizarTurnos();
        break;
  ...
  actionsEnemy(){
    ...
    for(let idx = 0; idx < this.Enemys.length; idx++){
      if(this.Enemys[idx].identificador == this.turnos.turnoActual) indice = idx;
    }
    ...
    switch(this.Enemys[indice].state){
      case 0 :
        ...
        if(Matematicas.distance(this.Enemys[indice].getCasilla() , this.protagonist.getCasilla()) == 1){
          ...
          if(this.Enemys[indice].getCasilla()[0] > this.protagonist.getCasilla()[0])this.Enemys[indice].cambiarOrientacion(0);//si enemigo por debajo de protagonista, que enemigo mire hacia arriba para
          if(this.Enemys[indice].getCasilla()[0] < this.protagonist.getCasilla()[0]) this.Enemys[indice].cambiarOrientacion(2);
          if(this.Enemys[indice].getCasilla()[1] > this.protagonist.getCasilla()[1]) this.Enemys[indice].cambiarOrientacion(3);
          if(this.Enemys[indice].getCasilla()[1] < this.protagonist.getCasilla()[1]) this.Enemys[indice].cambiarOrientacion(1);       
      ...
      case 2: 
        if(this.ataqueEnemigo(indice) == 0){this.turnos.actualizarTurnos();}
        break;
    }
  ...
  comprobarSiPuedeMoverse(c : Character){
  ...
  comprobarSiCogerObjeto(){
    ...
    for(let i = 0; i < this.pociones.length; i++){
      if(this.pociones[i].getCasilla()[0] == this.protagonist.getCasilla()[0] && this.pociones[i].getCasilla()[1] == this.protagonist.getCasilla()[1]){//estoy encima de una pocion
        if(this.protagonist.cogerPocion()){
          this.pociones.splice(i, 1);//la elimino
          this.interface.addTextLog("Poción cogida");
        }
        else this.interface.addTextLog("No se puede coger más pociones");
      }
    }

    for(let i = 0; i < this.flechas.length; i++){
      if(this.flechas[i].getCasilla()[0] == this.protagonist.getCasilla()[0] && this.flechas[i].getCasilla()[1] == this.protagonist.getCasilla()[1]){//estoy encima de una pocion
        if(this.protagonist.cogerFlecha()){
          this.flechas.splice(i, 1);//la elimino
          this.interface.addTextLog("Flecha cogida");
        }
        else this.interface.addTextLog("No se puede coger más flechas");
      }
    }
  ...
  comprobarSiEscalera(){
    ...
    switch(this.protagonist.orientacion){
      case 0 : posSig[0] -=1; break;
      case 1 : posSig[1] +=1; break;
      case 2 : posSig[0] +=1; break;
      case 3 : posSig[1] -=1; break;
    }
  ...
  atacar(){
    switch(this.stateAtacar){
      ...
      case 1 : 
         if(this.enemigoDañado != -1) this.Enemys[this.enemigoDañado].actualizarFrame();
         
         if(this.protagonist.state == 2){
            this.protagonist.actualizarFrame();
            if(this.protagonist.frameActual == this.protagonist.framesAnimaciones[this.protagonist.getState()]-1){
              this.stateAtacar = 2; 
              if(this.enemigoDañado != -1)this.Enemys[this.enemigoDañado].state = 0;
            }
         }
         else{
            this.arco.actualizarFrame();
  ...
  comprobarEnemigoDañado(){
    ...
    if(this.protagonist.orientacion == 0) casillaProta[0] -=1;
    if(this.protagonist.orientacion == 1) casillaProta[1] +=1;
    if(this.protagonist.orientacion == 2) casillaProta[0] +=1;
    if(this.protagonist.orientacion == 3) casillaProta[1] -=1;
  ...
  comprobarEnemigoDañadoArco(){
    ...
    for(let i = 0; i < 3; i++){
      if(casillasAfectadas[i][0] > 0 && casillasAfectadas[i][0] < this.map.tamMapa && casillasAfectadas[i][1] > 0 && casillasAfectadas[i][1] < this.map.tamMapa){//si no se sale del mapa
  ...
  stateAtaqueEnemigo = 0;
  ataqueEnemigo(indice : number){
  ...
  actualizarMovimientos(){
    ...
    this.Enemys.forEach(enemy => {
      if(enemy.getState() == 1) 
        if(enemy.andar(this.map) == 0){this.numeroPersonajeAndando--;}
    });
  }
...
function majeadorTeclado(e : any){
    if(clockControles.delta >= 0.2){
      clockControles.iniciar();
      console.log(e.keyCode)
      if(e.keyCode == 39){ tecla = 1; console.log("der")}
      else if(e.keyCode == 37){ tecla = 3;console.log("izq")}
      else if(e.keyCode == 38){ tecla = 0; console.log("arr")}
      else if(e.keyCode == 40){ tecla = 2;console.log("abaj")}
      else if(e.keyCode == 65){ tecla = 4;console.log("atacar")}
      else if(e.keyCode == 67){ tecla = 8;console.log("flecha")}
      else if(e.keyCode == 80){ tecla = 7;console.log("pocion")}
      else if(e.keyCode == 77){ tecla = 6;console.log("m de mapa")}
...
document.addEventListener("touchstart", (e) => {
  if(canvas2 != null){
    ...
    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.145,window.innerHeight*0.63]) <= window.innerWidth*0.05){//arriba
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 0;
    }
    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.215,window.innerHeight*0.765]) <= window.innerWidth*0.05){//der
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 1;
    }
    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.145,window.innerHeight*0.9]) <= window.innerWidth*0.05){//abajo
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 2;
    }
    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.065,window.innerHeight*0.765]) <= window.innerWidth*0.05){//izq
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 3;
    }

    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.92,window.innerHeight*0.87]) <= window.innerWidth*0.05){//atacar
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 4;
    }      
    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.8,window.innerHeight*0.87]) <= window.innerWidth*0.05){//potion
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 7;
    }
    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.92,window.innerHeight*0.68]) <= window.innerWidth*0.05){//flecha
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 8;
    }      

    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.87,window.innerHeight*0.17]) <= window.innerWidth*0.09){//mapa
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 6;
    }

    if( Matematicas.distance([e.touches[0].clientX,e.touches[0].clientY],[window.innerWidth*0.15,window.innerHeight*0.01]) <= window.innerWidth*0.09){//mapa
      game.interface.sombrearTouch(e.touches[0].clientX,e.touches[0].clientY,true);
      tecla = 9;
    }
...
document.addEventListener("touchend", (e) => {
  if(canvas2 != null){
    game.interface.sombrearTouch(0,0,false);//quita el sombreado
  }
}, false);
...
function comprobarAccionPulsada(){
      ...
      tecla = -1;
      if(teclacomprobada == -1){ return -1;}
      else if(teclacomprobada == 0){ return 0;}
      else if(teclacomprobada == 1){ return 1;}
      else if(teclacomprobada == 2){ return 2;}
      else if(teclacomprobada == 3){ return 3;}
      else if(teclacomprobada == 4){ return 4;}//atacar normal
      else if(teclacomprobada == 5){ return 5;}//pasar turno
      else if(teclacomprobada == 6){ return 6;}//mostrar mapa
