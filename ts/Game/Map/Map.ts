...
export class Mapa {
    ...
    tamHabitacionMax: number;
    tamHabitacionMin: number;

    numIntentos: number;

    numHabitacionesMin: number;

    HabitacionesCreadas: number;
    PosHabitacionesX: number[];
    PosHabitacionesY: number[];
    ...
    dividir(y: number, x: number, h: number, w: number) {
        ...
        if (h - y >= this.tamHabitacionMax && w - x >= this.tamHabitacionMax) {
            ...
            let horizontaloVertical = (this.numIntentos + (this.semilla + this.mapaNumero) + (this.HabitacionesCreadas * this.tamMapa)) % 2;//0 horizontal, 1 vertical
            ...
            if (horizontaloVertical == 0) {
                ...
                if (nuevaH > this.tamMapa) nuevaH = this.tamMapa - 1;
                nuevaW = w;
                nuevaY = nuevaH;
                nuevaX = x;
            }
                ...
                if (nuevaW > this.tamMapa) nuevaW = this.tamMapa - 1; 
                nuevaY = y;
                nuevaX = nuevaW;
            }
            this.numIntentos++;
    ...
    crearHabitacion(y: number, x: number, h: number, w: number) {
    ...
    crearCamino(hab1Y: number, hab2Y: number, hab1X: number, hab2X: number) {
    ...
    comprobarCasillaEsPared(y : any, x : any){
        if(this.cuadricula[y][x] == 999) return true;
        return false;
    }

    comprobarCasillaVacia(y : any, x : any){
        if(this.cuadricula[y][x] == 1) return true;
        return false;
    }

    comprobarCasillaOcupada(y : any, x : any){
        if(this.cuadricula[y][x] == 2) return true;
        return false;
    }   
    ...
    getCasillaVacia(){
        ...
        if(intentos >= 10) return [this.PosHabitacionesY[habitacion],this.PosHabitacionesX[habitacion]];
    ...
    liberarCasilla(casilla : any){
        this.cuadricula[casilla[0]][casilla[1]] = 1;
    }

    OcuparCasilla(casilla : any){
        this.cuadricula[casilla[0]][casilla[1]] = 2;
    }

    esDentroMapa(y: any, x: any){
        if(y >= 0 && y <= this.tamMapa && x >=0 && x <this.tamMapa) return true;
        return false;
    }
    ...
    esEscalera(y : any, x: any){
        if(this.cuadricula[y][x] == 3) return true;
        return false;
    }
