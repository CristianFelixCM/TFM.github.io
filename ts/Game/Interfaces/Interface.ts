export class Interface{
    ...
    drawInterfaceGamePlay(map : any, prota: any){
        ...
        if(this.pulsadoTouch) this.pintarSombrado();
    ...
    drawMiniMap(map : any, prota: any){
        ...
        for(let y = -2; y <= 2; y++){
            for(let x = -2; x <= 2; x++){
                if(prota.getCasilla()[0] + y >= 0 && prota.getCasilla()[1] + x >= 0 && prota.getCasilla()[0] + y < map.tamMapa && prota.getCasilla()[1] + x < map.tamMapa ) {//compruebo si no me salgo de las casillas
    ...
    showNumeroPocionesYFlechas(prota : any){
        let posY = 0;
        let posX = 0;
        if(this.isPC){
            this.context.font = this.canvas.width*0.02+"px Amita";
    ...
    showBarraVida(prota : any){
        let posY = 0;
        let posX = 0; 
        
        this.context.fillStyle = "red";
        if(this.isPC) this.context.fillRect(this.canvas.width*0.355, this.canvas.height*0.9, this.canvas.width*0.321 * (prota.HpActual/prota.Hp), this.canvas.height*0.055);
        else this.context.fillRect(this.canvas.width*0.343, this.canvas.height*0.865, this.canvas.width*0.321 * (prota.HpActual/prota.Hp), this.canvas.height*0.055);


        this.context.shadowColor="Black";
        this.context.shadowBlur=7;
        this.context.fillStyle = "White";
        if(this.isPC){ this.context.font = this.canvas.width*0.02+"px Amita"; posY = this.canvas.height*0.945; posX = this.canvas.width*0.475;}
        else {this.context.font = this.canvas.width*0.03+"px Amita"; posY = this.canvas.height*0.925; posX = this.canvas.width*0.44;}
    ...
    xPulsado = 0;
    yPulsado = 0;
    sombrearTouch(x:any ,y:any, pintar : boolean){
        this.xPulsado = x;
        this.yPulsado = y;
    ...
    pintarContadorFPS(){
       ...
       this.context.shadowBlur=7;
       this.context.fillStyle = "White";
       if(this.isPC){ this.context.font = this.canvas.width*0.02+"px Amita"; posY = this.canvas.height*0.1; posX = this.canvas.width*0.1;}
       else {this.context.font = this.canvas.width*0.03+"px Amita"; posY = this.canvas.height*0.1; posX = this.canvas.width*0.1;}
